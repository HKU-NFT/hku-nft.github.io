...
df_win = pd.read_csv(data_path).loc[:,'1':'9']
...
df_return = pd.read_csv(path.join(root_dir, 'T1C_Return.csv')).loc[:,'2':'10']
fig_3 = go.Figure()
...
fig_3.update_layout(title="Token average gross return",
                    ...
                    yaxis={"title": "Total trade count (including initial buy in)"})
fig_3.update_traces(textfont_size = 20, hovertemplate='Total trade count (including initial buy in): %{y} <br>Number of trade after initial buy in: %{x} th <br>Gross return: %{z} <extra></extra>')
#fig_2.show()
...
fig_4 = go.Figure()
fig_4.add_trace(go.Scatter(x = df_2a['day'], y = df_2a['Proj_traded'], name = "Number of project traded", mode = "lines"))
fig_4.add_trace(go.Scatter(x = df_2a['day'], y = df_2a['Token_traded'], name = "Number of token traded", mode = "lines"))
fig_4.update_layout(title = "Number of project/token traded",
                    xaxis = {"title":"Time"},
                    yaxis = {"title": "Traded amount"})
fig_4.update_traces(textfont_size = 20, hovertemplate = "Time: %{x}<br> Traded amount: %{y} <extra></extra>")
#fig_3.show()
#####chart for trade probability
df_prob = df_2a.loc[:,"trade_7d":"trade_365d"]
fig_5 = go.Figure()
                        ...
                        buttons=list(
                            ...
                            dict(label = 'trade_7d',
                                 ...
                                 args = [{'visible': [True, False, False, False, False]},
                                         ...
                                         {'showlegend':True}]),
                             dict(label = 'trade_14d',
                                 method = 'update',
                                 args = [{'visible': [False, True, False, False, False]},
                                         {'title': 'Trade in 14 Days'},
                                         {'showlegend':True}]),
                             dict(label = 'trade_91d',
                                 method = 'update',
                                 args = [{'visible': [False, False, True, False, False]},
                                         {'title': 'Trade in 91 Days'},
                                         {'showlegend':True}]),
                             dict(label = 'trade_182d',
                                 method = 'update',
